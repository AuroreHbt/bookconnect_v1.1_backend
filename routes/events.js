...
router.post("/addevent", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      return res.json({
        result: false,
        error: `Organisateur "${planner}" non trouvé.`,
      });
    }
...
router.get("/searcheventByUser/:planner", (req, res) => {
  ...
  User.findOne({ username: req.params.planner }).then((user) => {
    ...
    Event.findById(req.body.id)
      ...
      .then((event) => {
        if (!event) {
          ...
          res.json({
            result: false,
            error: "event can only be deleted by its planner",
          });
          return;
        }

        event
          .deleteOne({ _id: event._id })
          .then(() => {
